...
class LightGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        self.head_ff = args.head_ff
        self.tail_ff = args.tail_ff
        '''
        *********************************************************
        ...
        *********************************************************
        ...
        self.g_g_embeddings_pre_2 = tf.nn.embedding_lookup(self.weights['group_embedding_2'], self.groups)
        self.pos_u_g_embeddings_pre_2 = tf.nn.embedding_lookup(self.weights['user_embedding_2'], self.pos_users)
        self.neg_u_g_embeddings_pre_2 = tf.nn.embedding_lookup(self.weights['user_embedding_2'], self.neg_users)
    ...
    def _init_weights(self):
        ...
        if self.pretrain_data is None:
            ...
            all_weights['item_embedding'] = tf.Variable(initializer([self.n_items, self.emb_dim]),
                                                        name='item_embedding')
            all_weights['group_embedding_2'] = tf.Variable(initializer([self.n_groups, self.emb_dim]),
                                                         name='group_embedding_2')
            all_weights['user_embedding_2'] = tf.Variable(initializer([self.n_users, self.emb_dim]),
                                                        name='user_embedding_2')

            all_weights['item_embedding_2'] = tf.Variable(initializer([self.n_items, self.emb_dim]),
                                                        name='item_embedding_2')
    ...
    def _create_lightgcn_embed_general(self):
        ...
        if self.head_ff == 1:
            ...
            regularizer_type = tf.keras.regularizers.l2()
            Drop_out = tf.keras.layers.Dropout(0.2)
            user_ff_1 = tf.keras.layers.Dense(self.emb_dim,activation=activation_type,bias_regularizer=regularizer_type,kernel_regularizer=regularizer_type)
            user_ff_2 = tf.keras.layers.Dense(self.emb_dim,activation=activation_type,bias_regularizer=regularizer_type,kernel_regularizer=regularizer_type)
            weights_user_embedding_all = tf.concat([self.weights['user_embedding'], self.weights['user_embedding_2']], axis=1)
            weights_user_embedding = user_ff_1(weights_user_embedding_all)
            weights_user_embedding_2 = user_ff_2(weights_user_embedding_all)
            
            item_ff_1 = tf.keras.layers.Dense(self.emb_dim,activation=activation_type,bias_regularizer=regularizer_type,kernel_regularizer=regularizer_type)
            item_ff_2 = tf.keras.layers.Dense(self.emb_dim,activation=activation_type,bias_regularizer=regularizer_type,kernel_regularizer=regularizer_type)
            weights_item_embedding_all = tf.concat([self.weights['item_embedding'], self.weights['item_embedding_2']], axis=1)
            weights_item_embedding = item_ff_1(weights_item_embedding_all)
            weights_item_embedding_2 = item_ff_2(weights_item_embedding_all)
            
            group_ff_1 = tf.keras.layers.Dense(self.emb_dim,activation=activation_type,bias_regularizer=regularizer_type,kernel_regularizer=regularizer_type)
            group_ff_2 = tf.keras.layers.Dense(self.emb_dim,activation=activation_type,bias_regularizer=regularizer_type,kernel_regularizer=regularizer_type)
            weights_group_embedding_all = tf.concat([self.weights['group_embedding'], self.weights['group_embedding_2']], axis=1)
            weights_group_embedding = group_ff_1(weights_group_embedding_all)
            weights_group_embedding_2 = group_ff_2(weights_group_embedding_all)
        else:
            weights_user_embedding = self.weights['user_embedding']
            weights_user_embedding_2 = self.weights['user_embedding_2']
            weights_group_embedding = self.weights['group_embedding']
            weights_group_embedding_2 = self.weights['group_embedding_2']
            weights_item_embedding = self.weights['item_embedding']
            weights_item_embedding_2 = self.weights['item_embedding_2']
        if self.gat_type in ['user_side', 'both_side']:
            if self.att_ver == 1:  # one matrix
                atten_embedding = self.graph_atten(weights_user_embedding, self.R, self.gu_attention)
            elif self.att_ver == 2:  # GAT_mode
                atten_embedding = self.graph_atten_v2(weights_user_embedding, self.gu_attention_v2)
            elif self.att_ver == 3:
                atten_embedding = self.graph_atten_v3(weights_user_embedding, self.gu_attention)
            elif self.att_ver == 4:
                atten_embedding = self.graph_atten_v4(weights_user_embedding, self.R, self.gu_attention_vert,
                ...
                # print(weights_user_embedding.shape)
                # sys.exit(0)
                atten_embedding = self.graph_atten_v4(weights_user_embedding, self.R,
                                                      weights_user_embedding,
                                                      tf.transpose(weights_user_embedding))

            if self.att_concat:
                group_embedding = tf.concat([weights_group_embedding, atten_embedding], axis=1)
                all_embeddings = self.graph_conv(A_fold_hat, self.n_layers,
                                                 tf.concat([group_embedding, tf.concat(
                                                     [weights_user_embedding, weights_user_embedding],
                                                     axis=1)], axis=0))
            else:
                if args.att_norm == 0:
                    group_embedding = weights_group_embedding + self.att_coef * atten_embedding
                else:
                    group_embedding = tf.nn.l2_normalize(weights_group_embedding + self.att_coef * atten_embedding,axis=1)

                all_embeddings = self.graph_conv(A_fold_hat, self.n_layers,
                                                 tf.concat([group_embedding, weights_user_embedding], axis=0))
        else:
            all_embeddings = self.graph_conv(A_fold_hat, self.n_layers,
                                             tf.concat(
                                                 [weights_group_embedding, weights_user_embedding],
                                                 axis=0))
        ...
        if self.gat_type in ['item_side', 'both_side']:
            if self.att_ver == 1:
                atten_embedding_gi = self.graph_atten(weights_item_embedding, self.R_gi, self.gi_attention)
            elif self.att_ver == 2:
                atten_embedding_gi = self.graph_atten_v2(weights_item_embedding, self.gi_attention_v2)
            elif self.att_ver == 3:
                atten_embedding_gi = self.graph_atten_v3(weights_item_embedding, self.gi_attention)
            elif self.att_ver == 4:
                atten_embedding_gi = self.graph_atten_v4(weights_item_embedding, self.R_gi,
                ...
                atten_embedding_gi = self.graph_atten_v4(weights_item_embedding, self.R_gi,
                                                         weights_item_embedding,
                                                         tf.transpose(weights_item_embedding))

            if self.att_concat:
                group_embedding_gi = tf.concat([weights_group_embedding_2, atten_embedding_gi], axis=1)
                all_embeddings_gi = self.graph_conv(A_fold_hat_gi, self.n_layers,
                                                    tf.concat([group_embedding_gi, tf.concat(
                                                        [weights_item_embedding,
                                                         weights_item_embedding], axis=1)],
                                                              axis=0))
            else:
                if args.att_norm == 0:
                    group_embedding_gi = weights_group_embedding_2 + self.att_coef * atten_embedding_gi
                else:
                    group_embedding_gi = tf.nn.l2_normalize(weights_group_embedding_2 + self.att_coef * atten_embedding_gi,axis=1)
                all_embeddings_gi = self.graph_conv(A_fold_hat_gi, self.n_layers,
                                                    tf.concat([group_embedding_gi, weights_item_embedding],
                                                              axis=0))
        else:
            all_embeddings_gi = self.graph_conv(A_fold_hat_gi, self.n_layers,
                                                tf.concat(
                                                    [weights_group_embedding_2, weights_item_embedding],
                                                    axis=0))
        ...
        if self.gi_side == 1:
            if self.att_concat:
                all_embeddings_ui = self.graph_conv(A_fold_hat_ui, self.n_layers,
                                                    tf.concat([tf.concat([self.weights['user_embedding_2'],
                                                                          self.weights['user_embedding_2']], axis=1),
                                                               tf.concat([self.weights['item_embedding_2'],
                                                                          self.weights['item_embedding_2']], axis=1)],
                                                              axis=0))
            else:
                all_embeddings_ui = self.graph_conv(A_fold_hat_ui, self.n_layers,
                                                    tf.concat([weights_user_embedding_2,
                                                               weights_item_embedding_2],
                                                              axis=0))
            ...
            if self.tail_ff == 0:
                ...
                activation_type = 'relu'
                regularizer_type = tf.keras.regularizers.l2()
                user_ff = tf.keras.layers.Dense(self.emb_dim, activation=activation_type, bias_regularizer=regularizer_type,
                                      kernel_regularizer=regularizer_type)
                item_ff = tf.keras.layers.Dense(self.emb_dim, activation=activation_type, bias_regularizer=regularizer_type,
                                      kernel_regularizer=regularizer_type)
                group_ff = tf.keras.layers.Dense(self.emb_dim, activation=activation_type, bias_regularizer=regularizer_type,
                                      kernel_regularizer=regularizer_type)
                u_embeddings = tf.concat([u_g_embeddings, u_i_embeddings], 1)
                g_embeddings = tf.concat([g_u_embeddings, g_i_embeddings], 1)
                i_embeddings = tf.concat([i_g_embeddings, i_u_embeddings], 1)
                u_embeddings = user_ff(u_embeddings)
                g_embeddings = group_ff(g_embeddings)
                i_embeddings = item_ff(i_embeddings)
            elif self.tail_ff == 2:
                activation_type = 'relu'
                regularizer_type = tf.keras.regularizers.l2()
                user_ff_merge_1= tf.keras.layers.Dense(self.emb_dim, activation=activation_type, bias_regularizer=regularizer_type,
                                      kernel_regularizer=regularizer_type)
                item_ff_merge_1 = tf.keras.layers.Dense(self.emb_dim, activation=activation_type, bias_regularizer=regularizer_type,
                                      kernel_regularizer=regularizer_type)
                group_ff_merge_1 = tf.keras.layers.Dense(self.emb_dim, activation=activation_type, bias_regularizer=regularizer_type,
                                      kernel_regularizer=regularizer_type)
                user_ff_merge_2 = tf.keras.layers.Dense(self.emb_dim, activation=activation_type, bias_regularizer=regularizer_type,
                                      kernel_regularizer=regularizer_type)
                item_ff_merge_2 = tf.keras.layers.Dense(self.emb_dim, activation=activation_type, bias_regularizer=regularizer_type,
                                      kernel_regularizer=regularizer_type)
                group_ff_merge_2 = tf.keras.layers.Dense(self.emb_dim, activation=activation_type, bias_regularizer=regularizer_type,
                                      kernel_regularizer=regularizer_type)
                u_embeddings = tf.concat([user_ff_merge_1(u_g_embeddings), user_ff_merge_2(u_i_embeddings)], 1)
                g_embeddings = tf.concat([group_ff_merge_1(g_u_embeddings), group_ff_merge_2(g_i_embeddings)], 1)
                i_embeddings = tf.concat([item_ff_merge_1(i_g_embeddings), item_ff_merge_2(i_u_embeddings)], 1)
        elif self.gi_side == 0:
    ...
    def create_bpr_loss(self, groups, pos_users, neg_users):
        ...
        regularizer = tf.nn.l2_loss(self.g_g_embeddings_pre) + tf.nn.l2_loss(
            self.pos_u_g_embeddings_pre) + tf.nn.l2_loss(self.neg_u_g_embeddings_pre) + tf.nn.l2_loss(self.pos_u_g_embeddings_pre_2)+tf.nn.l2_loss(self.neg_u_g_embeddings_pre_2)
        # if self.aug_type == 5:
        #     regularizer = tf.nn.l2_loss(self.g_g_embeddings_pre) + tf.nn.l2_loss(
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    f.write(
        'top_k=%s, embed_size=%d, lr=%.4f, layer_size=%s, batch_size=%d, att_ver=%d, item_side=%d aug_type=%d, gat_type=%s, regs=%s, intra_emb_dim=%d, att_coef=%.3f, head_ff=%d, tail_ff=%d\n\t%s\n'
        % (args.Ks, args.embed_size, args.lr, args.layer_size, args.batch_size, args.att_ver, args.item_side,
           args.aug_type,
           args.gat_type, args.regs,
           args.intra_emb_dim,args.att_coef,args.head_ff,args.tail_ff, final_perf))
    f.close()
